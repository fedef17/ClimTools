            ...
            if np.sum(is_key) == 0:
                ...
                try:
                    if typ == list:
                        ...
                        # print(key, cose)
                        if typ == bool:
                            cose = [str_to_bool(lines[num_0+1].rstrip().split()[0])]
                        if typ == str:
                            cose = [lines[num_0+1].rstrip()]
                        if len(cose) == 1:
                            if cose[0] is None:
...
def read_from_txt(ifile, n_skip=0, dtype=float, first_column_header=True, sep=' ', n_stop=None, debug=False):
    ...
    with open(ifile, 'r') as ifi:
        lines = ifi.readlines()
        cose = readlines_to_matrix(lines, n_skip=n_skip, dtype=dtype,
                                   first_column_header=first_column_header, sep=sep, n_stop=n_stop, debug=debug)
...
def read_cmip6_data(varname, mip_table, experiment, model, sel_member='first', base_cart_cmip6=None, cmip_family_dirs=False, grid_dir=False, version_dir=False, extract_level_hPa=None, regrid_to_reference_file=None, adjust_nonstd_dates=True, verbose=False, netcdf4_read=True, sel_yr_range=None, select_area_first=False, select_season_first=False, area=None, season=None, remove_29feb=True):
    ...
    if len(all_mems) == 0:
        raise ValueError('NO data found for model {}, var {}, exp {}, miptab {}!'.format(
            model, varname, experiment, mip_table))
    ...
    if sel_member == 'all':
        ...
        for member in all_mems:
            ...
            if os.path.exists(pathok):
                ...
                if len(listafil) > 0:
                    var, coords, aux_info = read_xr(listafil, extract_level_hPa=extract_level_hPa, select_var=varname, regrid_to_reference_file=regrid_to_reference_file, adjust_nonstd_dates=True, verbose=verbose,
                                                    netcdf4_read=netcdf4_read, sel_yr_range=sel_yr_range, select_area_first=select_area_first, select_season_first=select_season_first, area=area, season=season, remove_29feb=remove_29feb)
        ...
        if sel_member == 'first':
            ...
            if member not in all_mems:
                raise ValueError('NO data found for member {} of model {}, var {}, exp {}, miptab {}!'.format(
                    member, model, varname, experiment, mip_table))
...
def create_xr_grid(lats, lons, gridname='grid'):
    ...
    data_vars = dict([('{}.lat'.format(gridname), (('lon', 'lat'), mgri[0])),
                     ('{}.lon'.format(gridname), (('lon', 'lat'), mgri[1]))])
...
def regrid_dataset(dataset, lats=None, lons=None, regrid_to_reference=None, regrid_to_deg=2.5, regrid_scheme='bilinear', ignore_degenerate=True):
    ...
    if regrid_to_reference is not None:
        ...
        regrid_to_reference = xr.Dataset(
            {'lat': (['lat'], np.arange(-90, 90.1, regrid_to_deg)), 'lon': (['lon'], np.arange(0, 360, regrid_to_deg)), })

    print('Regridding...')
    uno = datetime.now()
    regridder = xe.Regridder(dataset, regrid_to_reference, method=regrid_scheme,
                             extrap_method="nearest_s2d", ignore_degenerate=ignore_degenerate)
...
def read_xr(ifile, extract_level_hPa=None, select_var=None, regrid_to_reference=None, regrid_to_deg=None, regrid_to_reference_file=None, regrid_scheme='bilinear', convert_units_to=None, verbose=True, keep_only_maxdim_vars=True, year_range=None):
    ...
    if type(ifile) in [list, np.ndarray]:
        ...
        if ifile[0][-4:] == '.grb' or ifile[0][-5:] == '.grib':
            engine = 'cfgrib'
        pino = xr.open_mfdataset(ifile, use_cftime=True, engine=engine)
    else:
        if ifile[-4:] == '.grb' or ifile[-5:] == '.grib':
            engine = 'cfgrib'
        ...
        if len(datacoords['level']) == 1:
            ...
            if extract_level_hPa is not None:
                try:
                    ...
                    raise ValueError('Level {} hPa not found among: '.format(extract_level_hPa) +
                                     (len(datacoords['level'])*'{}, ').format(*datacoords['level']))
...
def read_ensemble_xD(file_list, extract_level=None, select_var=None, pressure_in_Pa=True, force_level_units=None, verbose=True, keep_only_Ndim_vars=True):
    ...
    dates_all = []
    for fi in file_list:
        vars, datacoords, aux_info = readxDncfield(fi, extract_level=extract_level, select_var=select_var, pressure_in_Pa=pressure_in_Pa,
                                                   force_level_units=force_level_units, verbose=verbose, keep_only_Ndim_vars=keep_only_Ndim_vars)
...
def readxDncfield(ifile, extract_level=None, select_var=None, pressure_in_Pa=True, force_level_units=None, verbose=True, keep_only_Ndim_vars=True):
    ...
    for nam in dimensions:
        if nam in variab_names:
            variab_names.remove(nam)
    if verbose:
        print('Variables: {}\n'.format(variab_names))
...
def sel_season_xr(dset, season, calc_average=False):
    ...
    if season in month_seq and len(season) > 1:
        ...
        raise ValueError(
            'season not understood, should be in DJF, JJA, ND,... format or the short 3 letters name of a month (Jan, Feb, ...)')
...
def sel_season(var, dates, season, cut=True, remove_29feb=True):
    ...
    if cut:
        ...
        if (12 in indxs) and (1 in indxs):
            ...
            # 0 is ok, I look for the beginning of the next (incomplete) season!
            end_cond = (years == years[-1]) & (months == indxs[0])
            if np.sum(end_cond):
...
def daily_climatology(var, dates, window, refyear=2001, force_daily=False):
    """
    ...
    if not force_daily and not check_daily(dates):
...
def local_lineartrend_climate(lat, lon, var, dates, season, print_trend=True, remove_global_trend=False, global_deg=3, global_area='global'):
    ...
    if remove_global_trend:
        var, coeffs, var_regional, dates = remove_global_polytrend(
            lat, lon, var, dates, season, deg=global_deg, area=global_area)
...
def seasonal_climatology(var, dates=None, season=None, dates_range=None, cut=True, percentiles=False, allseasons=['DJF', 'MAM', 'JJA', 'SON', 'year']):
    ...
    if isinstance(var, xr.DataArray):
        ...
        seas_clim = xr.Dataset(data_vars=dict(seamean=(dims, seas_mean), seastd=(dims, seas_std),
                               seap90=(dims, seas_p90), seap10=(dims, seas_p10)), coords=coords)
...
def seasonal_set(var, dates=None, season=None, dates_range=None, cut=True, seasonal_stat=None, verbose=False):
    ...
    try:
        ...
        if verbose:
            if len(lengths) > 1:
                for cos in all_dates_seas:
                    print(len(cos), cos[0], cos[-1])
...
def bootstrap(var, dates, season, y=None, apply_func=None, func_args=None, n_choice=30, n_bootstrap=100):
    ...
    var_set, dates_set = seasonal_set(var, dates, season)
    if y is not None:
        y_set, _ = seasonal_set(y, dates, season)
    ...
    for i in range(n_bootstrap):
        ...
        if y is not None:
            y_ok = np.concatenate(y_set[ok_yea])
...
def running_mean(var, wnd, remove_nans=False, cyclic=False):
    ...
    if var.ndim == 1:
        ...
        if remove_nans:
            rollpi_temp = rollpi_temp[~np.isnan(rollpi_temp)]
...
def anomalies_daily_detrended(var, dates, climate_mean=None, dates_climate_mean=None, window_days=5, window_years=20, step_year=5):
    ...
    if climate_mean is None or dates_climate_mean is None:
        climate_mean, dates_climate_mean = trend_daily_climat(
            var, dates, window_days=window_days, window_years=window_years, step_year=step_year)

    if len(climate_mean) == 0:
        raise ValueError(
            'ERROR in calculation of detrended climatology. Too few years? Try lowering wnd_years or set detrending to False')
    ...
    for yea in np.unique(years):
        ...
        var_anom = anomalies_daily(var[okye], dates[okye], climate_mean=climate_mean[yearef],
                                   dates_climate_mean=dates_climate_mean[yearef], window=window_days)
...
def anomalies_monthly_detrended(var, dates, climate_mean=None, dates_climate_mean=None, window_years=20, step_year=5):
    ...
    for yea in np.unique(dates_pdh.year):
        ...
        var_anom = anomalies_monthly(var[okye], dates[okye], climate_mean=climate_mean[yearef],
                                     dates_climate_mean=dates_climate_mean[yearef])
        var_anom_tot.append(var_anom)
            ...
            for la in range(var_ens[0].shape[1]):
                for lo in range(var_ens[0].shape[2]):
                    slope, intercept, r_value, p_value, std_err = stats.linregress(
                        list(range(var_ens[0].shape[0])), var_ens[i][:, la, lo])
    ...
    if latitude is not None:
        ...
        if not masked:
            ...
            if np.any(nans):
                ...
                xoknan = xok[~nans]
                yoknan = yok[~nans]
                corrcoef = np.cov(xoknan, yoknan, aweights=weights)/np.sqrt(np.cov(xoknan, xoknan,
                                                                                   aweights=weights) * np.cov(yoknan, yoknan, aweights=weights))
            else:
                weights = weights.flatten()
                corrcoef = np.cov(xok, yok, aweights=weights)/np.sqrt(np.cov(xok, xok,
                                                                             aweights=weights) * np.cov(yok, yok, aweights=weights))
        else:
...
def match_pc_sets(pcset_ref, pcset, latitude=None, verbose=False, bad_matching_rule='rms_mean_w_pos_patcor', matching_hierarchy=None):
    ...
    if len(np.unique([jmin, jmin2, jpathi, jrmshi, jrmshi_wpp])) > 1:
        print('WARNING: bad matching. Best permutation with rule rms_mean is {}, with patcor_mean is {}, with rms_hierarchy is {}, with patcor_hierarchy is {}, with rms_mean_w_pos_patcor is {}'.format(
            perms[jmin], perms[jmin2], perms[jrmshi], perms[jpathi], perms[jrmshi_wpp]))
...
def composites_regimes_daily(lat, lon, field, dates_field, labels, dates_labels, comp_moment='mean', comp_percentile=90, calc_anomaly=True, detrend_global=False, deg_dtr=1, area_dtr='global', detrend_local_linear=False):
    ...
    if calc_anomaly:
        var_anom = remove_seasonal_cycle(field, lat, lon, dates_field, detrend_global=detrend_global,
                                         deg_dtr=deg_dtr, area_dtr=area_dtr, detrend_local_linear=detrend_local_linear)
...
def calc_regime_transmatrix(n_ens, indices_set, dates_set, max_days_between=3, filter_longer_than=1, filter_shorter_than=None):
    ...
    if n_ens > 1:
        for ens in range(n_ens):
            trans_matrix_ens, _ = count_regime_transitions(
                indices_set[ens], dates_set[ens], max_days_between=max_days_between, filter_longer_than=filter_longer_than, filter_shorter_than=filter_shorter_than)
            if ens == 0:
                trans_matrix = trans_matrix_ens
            else:
                trans_matrix += trans_matrix_ens
    else:
        trans_matrix, _ = count_regime_transitions(indices_set, dates_set, max_days_between=max_days_between,
                                                   filter_longer_than=filter_longer_than, filter_shorter_than=filter_shorter_than)
...
def find_transition_pcs(n_ens, indices_set, dates_set, pcs_set, fix_length=2, filter_longer_than=1, filter_shorter_than=None, skip_persistence=False, only_direct=False):
    ...
    for ens in range(n_ens):
        # print('ens member {}\n'.format(ens))
        trans_matrix_ens, trans_matrix_nums_ens = count_regime_transitions(
            indices_set[ens], dates_set[ens], max_days_between=max_days_between, filter_longer_than=filter_longer_than, filter_shorter_than=filter_shorter_than)
        if ens == 0:
...
def count_regime_transitions(indices, dates, filter_longer_than=1, filter_shorter_than=None, max_days_between=3):
    ...
    resid_times, dates_reg, num_reg = calc_regime_residtimes(
        indices, dates=dates, count_incomplete=True, skip_singleday_pause=skip_singleday_pause)
    ...
    for num, day_in, day_out, lenday, reg in zip(range(n_tot), all_nums_ini, all_nums_fin, all_rsd, all_clus):
        ...
        if day_new - day_out > max_days_between:
            continue
...
def change_of_base_matrix(old_basis, new_basis):
    ...
    # le cui COLONNE sono i vettori della base nuova nelle coordinate della vecchia
    new_basis_comp = np.array(new_basis_comp).T
...
def makeRectangle(area, npo=40, lon_type='0-360'):
    ...
    lonW, lonE, latS, latN = area
    rlats = np.concatenate([np.linspace(latS, latN, npo), np.tile(latN, npo),
                           np.linspace(latN, latS, npo), np.tile(latS, npo)])
    if lonW < lonE:
        rlons = np.concatenate([np.tile(lonW, npo), np.linspace(lonW, lonE, npo),
                               np.tile(lonE, npo), np.linspace(lonE, lonW, npo)])
    else:
        rlons = np.concatenate([np.tile(lonW, npo), np.linspace(lonW, lon_border, npo//2), np.linspace(lon_border-360., lonE, npo-npo//2),
                               np.tile(lonE, npo), np.linspace(lonE, lon_border-360., npo//2), np.linspace(lon_border, lonW, npo-npo//2)])
...
def color_set(n, cmap='nipy_spectral', bright_thres=None, full_cb_range=False, only_darker_colors=False, use_seaborn=False, sns_palette='hls'):
    ...
    if not use_seaborn:
        ...
        if sns_palette == 'Paired':
            max_sns_paired = 10
            other_colors = ['burlywood', 'saddlebrown', 'palegoldenrod', 'goldenrod',
                            'lightslategray', 'darkslategray', 'orchid', 'darkmagenta']
...
def plot_mapc_on_ax(ax, data, lat, lon, proj, cmappa, cbar_range, n_color_levels=21, draw_contour_lines=False, n_lines=8, bounding_lat=None, plot_margins=None, add_hatching=None, hatch_styles=['', '', '...'], hatch_levels=[0.2, 0.8], colors=None, line_color='k', clevels=None, add_rectangles=None, draw_grid=True, alphamap=1.0, plot_type='filled_contour', verbose=False, lw_contour=0.5, add_contour_field=None, add_vector_field=None, quiver_scale=None, vec_every=2, add_contour_same_levels=True, add_contour_plot_anomalies=False, add_contour_lines_step=None, add_contour_range=None, extend_opt='both', color_norm=None, add_contour_regrid_shape=30, vector_plot='streamline'):
    ...
    if plot_type == 'filled_contour':
        ...
        if nskip == 0:
            nskip = 1
        map_plot = ax.contour(xi, yi, data, clevels[::nskip], colors=line_color,
                              transform=ccrs.PlateCarree(), linewidths=lw_contour)
    ...
    if add_contour_field is not None:
        ...
        if add_contour_lines_step is not None:
        ...
        if add_contour_same_levels:
            ...
            if add_contour_range is None:
                ...
                cb2 = add_contour_range

            if add_contour_lines_step is None:
                levs = np.linspace(cb2[0], cb2[1], n_lines)
            else:
                if add_contour_plot_anomalies:
                    levs = np.append(np.arange(0, cb2[0]-add_contour_lines_step/2, -add_contour_lines_step)
                                     [::-1], np.arange(0, cb2[1]+add_contour_lines_step/2, add_contour_lines_step)[1:])
                else:
        ...
        map_plot_lines = ax.contour(xi, yi, add_contour_field, levs, colors='black',
                                    transform=ccrs.PlateCarree(), linewidths=lw_contour)
    ...
    if add_hatching is not None:
        ...
        hatch = ax.contourf(xi, yi, add_hatching, levels=hatch_levels, transform=ccrs.PlateCarree(),
                            hatches=hatch_styles, colors='none', extend='both')
    ...
    if add_vector_field is not None:
        ...
        if vector_plot == 'streamline':
            ...
            vecfi = ax.quiver(xi[::vec_every, ::vec_every], yi[::vec_every, ::vec_every], u_ok[::vec_every, ::vec_every], v_ok[::vec_every, ::vec_every], angles='xy', linewidth=0.5,
                              scale=quiver_scale, transform=ccrs.PlateCarree(), regrid_shape=add_contour_regrid_shape)  # quiver_scale è inversamente proporzionale alla lunghezza delle frecce
            vecfi._init()
...
def def_projection(visualization, central_lat_lon=None, bounding_lat=0):
    ...
    if visualization == 'standard' or visualization == 'Standard':
        ...
        raise ValueError(
            'visualization {} not recognised. Only standard, Npolar (or polar), Spolar, Nstereo (or stereo), Sstereo accepted'.format(visualization))
...
def primavera_boxplot_on_ax(ax, allpercs, model_names, colors, edge_colors, vers, ens_names, ens_colors, wi=0.6, plot_mean=True):
    ...
    for iii, (mod, col, vv, mcol) in enumerate(zip(model_names, colors, vers, edge_colors)):
        ...
        boxlist = []
        box = {'med': allpercs['p50'][iii], 'q1': allpercs['p25'][iii], 'q3': allpercs['p75'][iii],
               'whislo': allpercs['p10'][iii], 'whishi': allpercs['p90'][iii], 'showfliers': False}
        boxlist.append(box)
        ...
        bxp_kwargs = {'boxprops': boxprops, 'whiskerprops': whiskerprops, 'capprops': capprops,
                      'medianprops': medianprops, 'patch_artist': True, 'showfliers': False}
    ...
    for kak, col in zip(ens_names, ens_colors):
        ...
        boxlist = []
        box = {'med': mean_vals[('p50', kak)], 'q1': mean_vals[('p25', kak)], 'q3': mean_vals[('p75', kak)],
               'whislo': mean_vals[('p10', kak)], 'whishi': mean_vals[('p90', kak)], 'showfliers': False}
        boxlist.append(box)
...
def boxplot_on_ax(ax, allpercs, model_names, colors, edge_colors=None, versions=None, positions=None, wi=0.5, plot_ensmeans=True, ens_colors=['indianred'], ens_names=['CMIP6'], obsperc=None, obs_color='black', obs_name='ERA', plot_mean=True, plot_minmax=False, plot_all=True):
    ...
    if plot_all:
        for iii, (pos, mod, col, vv, mcol) in enumerate(zip(positions, model_names, colors, versions, edge_colors)):
            ...
            if plot_mean:
                ax.scatter(pos, allpercs['mean'][iii], color=mcol, marker='o', s=20)
    ...
    if obsperc is not None:
        ...
        boxlist = []
        box = {'med': obsperc['p50'], 'q1': obsperc['p25'], 'q3': obsperc['p75'],
               'whislo': obsperc['p10'], 'whishi': obsperc['p90'], 'showfliers': False}
        boxlist.append(box)
        ...
        if plot_mean:
            ax.scatter(positions[iii], obsperc['mean'], color=obs_color, marker='o', s=20)
    ...
    if plot_ensmeans:
        ...
        for kak, col, pos in zip(ens_names, ens_colors, positions[iii:]):
            ...
            boxlist = []
            box = {'med': mean_vals[('p50', kak)], 'q1': mean_vals[('p25', kak)], 'q3': mean_vals[(
                'p75', kak)], 'whislo': mean_vals[('p10', kak)], 'whishi': mean_vals[('p90', kak)], 'showfliers': False}
            boxlist.append(box)
            ...
            boxplo = ax.bxp(boxlist, [pos], [wi], **bxp_kwargs)
            if plot_mean:
                ax.scatter(pos, mean_vals[('mean', kak)], color=col, marker='o', s=20)
...
def violinplot_on_ax(ax, alldists, names=None, colors=None, edge_colors=None, positions=None, wi=0.4, plot_ensmeans=True, ens_colors=['indianred'], ens_names=['CMIP6'], obsdist=None, obs_color='black', obs_name='ERA', plot_mean=False, plot_minmax=False):
    ...
    parts = ax.violinplot(alldists, positions=positions, widths=wi, showmeans=plot_mean,
                          showextrema=plot_minmax, showmedians=True)  # , quantiles=[0.25, 0.75]
...
def plot_map_contour(data, lat=None, lon=None, filename=None, visualization='standard', central_lat_lon=None, cmap='RdBu_r', title=None, xlabel=None, ylabel=None, cb_label=None, cbar_range=None, plot_anomalies=False, n_color_levels=21, draw_contour_lines=False, n_lines=5, line_color='k', color_percentiles=(0, 100), figsize=(8, 6), bounding_lat=30, plot_margins=None, add_rectangles=None, draw_grid=True, plot_type='filled_contour', verbose=False, lw_contour=0.5, add_contour_field=None, add_vector_field=None, quiver_scale=None, add_hatching=None, hatch_styles=['', '', '...'], vec_every=2, add_contour_same_levels=False, add_contour_plot_anomalies=False, add_contour_lines_step=None, add_contour_range=None, extend_opt='both', color_norm=None, clevels=None, return_ax=False, add_contour_regrid_shape=30, vector_plot='streamline'):
    ...
    map_plot = plot_mapc_on_ax(ax, data, lat, lon, proj, cmappa, cbar_range, n_color_levels=n_color_levels, draw_contour_lines=draw_contour_lines, n_lines=n_lines, bounding_lat=bounding_lat, plot_margins=plot_margins, add_rectangles=add_rectangles, draw_grid=draw_grid, plot_type=plot_type, verbose=verbose, lw_contour=lw_contour, add_contour_field=add_contour_field, add_vector_field=add_vector_field, quiver_scale=quiver_scale,
...
def plot_double_sidebyside(data1, data2, lat, lon, filename=None, visualization='standard', central_lat_lon=None, cmap='RdBu_r', title=None, xlabel=None, ylabel=None, cb_label=None, stitle_1='data1', stitle_2='data2', cbar_range=None, plot_anomalies=False, n_color_levels=21, draw_contour_lines=False, n_lines=5, color_percentiles=(0, 100), use_different_grids=False, bounding_lat=30, plot_margins=None, add_rectangles=None, draw_grid=True, plot_type='filled_contour', verbose=False, lw_contour=0.5):
    ...
    map_plot = plot_mapc_on_ax(ax, data1, lat1, lon1, proj, cmappa, cbar_range, n_color_levels=n_color_levels, draw_contour_lines=draw_contour_lines, n_lines=n_lines,
                               bounding_lat=bounding_lat, plot_margins=plot_margins, add_rectangles=add_rectangles, draw_grid=draw_grid, plot_type=plot_type, verbose=verbose, lw_contour=lw_contour)
...
def plot_triple_sidebyside(data1, data2, lat, lon, filename=None, visualization='standard', central_lat_lon=None, cmap='RdBu_r', title=None, xlabel=None, ylabel=None, cb_label=None, stitle_1='data1', stitle_2='data2', cbar_range=None, plot_anomalies=False, n_color_levels=21, draw_contour_lines=False, n_lines=5, color_percentiles=(0, 100), use_different_grids=False, bounding_lat=30, plot_margins=None, add_rectangles=None, draw_grid=True, plot_type='filled_contour', verbose=False, lw_contour=0.5):
    ...
    map_plot = plot_mapc_on_ax(ax, diff, lat_ok, lon_ok, proj, cmappa, cbar_range, n_color_levels=n_color_levels, draw_contour_lines=draw_contour_lines, n_lines=n_lines,
                               bounding_lat=bounding_lat, plot_margins=plot_margins, add_rectangles=add_rectangles, draw_grid=draw_grid, plot_type=plot_type, verbose=verbose, lw_contour=lw_contour)
...
def plot_multimap_contour(dataset, lat=None, lon=None, filename=None, max_ax_in_fig=30, number_subplots=False, cluster_labels=None, cluster_colors=None, repr_cluster=None, visualization='standard', central_lat_lon=None, cmap='RdBu_r', title=None, xlabel=None, ylabel=None, cb_label=None, cbar_range=None, plot_anomalies=False, n_color_levels=21, draw_contour_lines=False, n_lines=5, subtitles=None, color_percentiles=(0, 100), fix_subplots_shape=None, figsize=(15, 12), bounding_lat=30, plot_margins=None, add_rectangles=None, draw_grid=True, reference_abs_field=None, plot_type='filled_contour', clevels=None, verbose=False, lw_contour=0.5, add_contour_field=None, add_vector_field=None, quiver_scale=None, vec_every=2, add_hatching=None, hatch_styles=['', '', '...'], add_contour_same_levels=True, add_contour_plot_anomalies=False, add_contour_lines_step=None, add_contour_range=None, use_different_cbars=False, use_different_cmaps=False, color_norm=None, use_different_grids=False, fig_external=None, axs_external=None, cbar_bottomspace=0.08):
    ...
    if use_different_grids:
        if len(dataset) != len(lat) or len(dataset) != len(lon):
            raise ValueError('use_different_grids set, but lat/lon are not {}-elements lists'.format(len(dataset)))
    ...
    for i in range(num_figs):
        ...
        for nens in range(numens_ok*i, numens_ok*(i+1)):
            ...
            map_plot = plot_mapc_on_ax(ax, dataset[nens], lat_ok, lon_ok, proj, cma, cba, n_color_levels=n_color_levels, draw_contour_lines=draw_contour_lines, n_lines=n_lines, bounding_lat=bounding_lat, plot_margins=plot_margins, add_rectangles=add_rectangles, draw_grid=draw_grid, plot_type=plot_type, verbose=verbose, clevels=clev, lw_contour=lw_contour, add_contour_field=add_contour_field[nens],
                                       add_vector_field=add_vector_field[nens], quiver_scale=quiver_scale, vec_every=vec_every, add_hatching=add_hatching[nens], hatch_styles=hatch_styles, add_contour_same_levels=add_contour_same_levels, add_contour_plot_anomalies=add_contour_plot_anomalies, add_contour_lines_step=add_contour_lines_step, add_contour_range=add_contour_range, color_norm=color_norm)

            if number_subplots:
                subtit = nens
                title_obj = plt.text(-0.05, 1.05, subtit, horizontalalignment='center', verticalalignment='center',
                                     transform=ax.transAxes, fontsize=20, fontweight='bold', zorder=20)
        ...
        if not (use_different_cbars or use_different_cmaps):
            ...
            cax = plt.axes([0.1, cbar_bottomspace, 0.8, 0.03])
...
def plot_lat_crosssection(data, lat=None, levels=None, filename=None, ax=None, cmap='RdBu_r', title=None, xlabel=None, ylabel=None, cb_label=None, cbar_range=None, plot_anomalies=False, n_color_levels=21, draw_contour_lines=False, n_lines=5, color_percentiles=(0, 100), figsize=(10, 6), pressure_levels=True, set_logscale_levels=False, return_ax=False, ylim=None, add_contour_field=None, add_contour_same_levels=True, add_contour_plot_anomalies=False, add_contour_lines_step=None, add_contour_range=None, lw_contour=0.5):
    """
    ...
    if lat is None or levels is None:
        if isinstance(data, xr.DataArray):
            if 'lat' in data.coords:
                lat = data.lat.values
                levels = data.lev.values
            elif 'rlat' in data.coords:
                lat = data.rlat.values
                levels = data.lev.values
            else:
                lat = data.latitude.values
                levels = data.lev.values
            ...
            raise ValueError('lat/levels not specified')
    ...
    if draw_contour_lines:
        map_plot_lines = ax.contour(xi, yi, data, n_lines, colors='k', linewidth=lw_contour)
    ...
    if add_contour_field is not None:
        ...
        map_plot_lines = ax.contour(xi, yi, add_contour_field, levs, colors='k', linewidths=lw_contour)
...
def plot_animation_map(maps, lat, lon, labels=None, fps_anim=5, title=None, filename=None, visualization='standard', central_lat_lon=None, cmap='RdBu_r', xlabel=None, ylabel=None, cb_label=None, cbar_range=None, plot_anomalies=False, n_color_levels=21, draw_contour_lines=False, n_lines=5, color_percentiles=(0, 100), figsize=(8, 6)):
    ...
    def update_lines(num):
        ...
        plot_mapc_on_ax(ax, mapa, lat, lon, proj, cmappa, cbar_range, n_color_levels=n_color_levels, draw_contour_lines=draw_contour_lines, n_lines=n_lines,
                        bounding_lat=bounding_lat, plot_margins=plot_margins, add_rectangles=add_rectangles, draw_grid=draw_grid, plot_type=plot_type)
...
def Taylor_plot_EnsClus(models, observation, filename, title=None, label_bias_axis=None, label_ERMS_axis=None, show_numbers=True, only_numbers=False, colors_all=None, cluster_labels=None, cluster_colors=None, repr_cluster=None, verbose=True):
    ...
    for sig in [1., 2., 3.]:
        circle = plt.Circle((sigma_obs, 0.), sig*sigma_obs, transform=ax.transData._b,
    ...
    if verbose and cluster_labels is not None:
        ...
        print('The member with largest correlation coefficient is member {} of cluster {}\n'.format(
            max_corr, cluster_labels[max_corr]))
...
def Taylor_plot(models, observation, latitude=None, filename=None, ax=None, title=None, label_bias_axis=None, label_ERMS_axis=None, colors=None, markers=None, only_first_quarter=False, legend=True, marker_edge=None, labels=None, obs_label=None, mod_points_size=60, obs_points_size=90, enlarge_rmargin=True, relative_std=True, max_val_sd=None, plot_ellipse=False, ellipse_color='blue', alpha_markers=1.0):
    ...
    if title is not None:
        ax.text(0.95, 0.9, title, horizontalalignment='center', verticalalignment='center',
                rotation='horizontal', transform=ax.transAxes, fontsize=20)
        # ax.set_title(title)
    # ax.set_facecolor(bgcol)
    ...
    for ang, sig, col, sym, lab in zip(angles, sigmas_pred, colors, markers, labels):
        ax.scatter(ang, sig, s=mod_points_size, color=col, marker=sym,
                   edgecolor=marker_edge, label=lab, clip_on=False, alpha=alpha_markers)
    # i = 0
    # for ang, sig, col, sym, lab in zip(angles, sigmas_pred, colors, markers, labels):
    #     if i < 10:
    ...
    for sig in [1., 2., 3.]:
        circle = plt.Circle((np.mean(observation), 0.), sig*sigma_obs_abs, fill=False,
                            edgecolor='black', linestyle='--', clip_on=True)
        ax.add_artist(circle)
        ...
        if not np.all([isclose(ck, 1.0) for ck in check_all]):
            ...
            for mod in model_names:
                if mod == reference:
                    ...
                    nu_pcs_set[mod] = project_set_on_new_basis(
                        results[mod]['pcs'], results[mod][eofkey], results[reference][eofkey])
        else:
    ...
    for reg in range(n_clus):
        for i, proj in enumerate(eof_proj):
            ...
            for mod, col in zip(model_names, colors):
                ...
                if mod == reference:
                    lw = 2
                plot_regime_pdf_onax(ax, labels, pcs, reg, eof_proj=proj, color=col, fig_label=mod, levels=levels,
                                     plot_centroid=plot_centroids, eof_axis_lim=eof_axis_lim, lw=lw, xi_grid=xi_grid, yi_grid=yi_grid)
            ax.set_xlabel('EOF {}'.format(proj[0]))
            ax.set_ylabel('EOF {}'.format(proj[1]))
...
def custom_legend(fig, colors, labels, markers=None, loc='lower center', ncol=None, fontsize=18, bottom_margin_per_line=0.05, add_space_below=0.03):
    ...
    if markers is None:
        ...
        proxy = [mpl.lines.Line2D([], [], color=col, marker=mark, linestyle='None', markersize=8)
                 for col, mark in zip(colors, markers)]
