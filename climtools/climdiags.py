...
def WRtool_yearround(ds, geopot_var, start_date = None, end_date = None, area = 'EAT', climatology_window_length = 90,
                       low_pass_filter=False, low_pass_window=10, detrend=False, detrend_min_periods=50, detrend_time=1825,
                       eof_calc=False, numclus = 7, numpcs = 7, kmeans=False, final_clus = 8):
    ...
    if area is not None:
        ...
        ds_dom = ds.where((ds.lat > latS) & (ds.lat < latN) & (ds.lon > lonW) & (ds.lon < lonE),drop=True)

    # First, ensure the time is in datetime64 (optional depending on your data)
    # If already in datetime64, skip this step
    if isinstance(ds_dom['time'].values[0], cftime.DatetimeNoLeap):
        ds_dom['time'] = ds_dom.indexes['time'].to_datetimeindex()
    # eliminate leap days from time coordinate
    ds_dom = ds_dom.sel(time=~((ds_dom['time.month'] == 2) & (ds_dom['time.day'] == 29)))
    print(ds_dom)
    # we go from the dataset to the geopotential height variable in the xarray
    geopot = ds_dom[geopot_var]
    #geopot_var = ['z'][:,:,:] # name of the geopotential height in the ds

#######################################

    # climatology
    print("Computing climatology")
    clim = geopot.rolling(time = climatology_window_length, center=True).mean().groupby('time.dayofyear').mean('time').compute()   
    # Compute geopotential height anomalies subtracting the reference climatology for the selected domain
    print("Computing anomalies")
    anom = geopot.groupby('time.dayofyear') - clim
    anom = anom.compute()
    
########################################

    # Optional low pass filter
    # if the option low_pass_filter = False it skips this part and goes directly to the normalization
    if low_pass_filter:
        print("Low-pass filter")
        # transform xarray in numpy array - necessary to use ctl.butter_filter function
        anom_np = anom.values
        # apply filter
        anom_np = ctl.butter_filter(anom_np, low_pass_window, filtype='lowpass', axis=0, order=5)

        # reconstruct xarray.DataArray - easier to have xarray tham numpy for the following operations
        time_data = geopot['time']
        anom = xr.DataArray(anom_np, coords={'time': time_data, 'lat': geopot['lat'], 'lon': geopot['lon']},
                            dims=['time', 'lat', 'lon'])
        print(anom)

#######################################
    # Normalization process - normalizing for the standard deviation of the geopotential height anomalies
    # necessary to obtain year-round regimes
    print("Normalization procedure")
    # computation of the rolling standard deviation grouping the time by day
    rolling_std = anom.groupby('time.dayofyear').std('time').compute()
    print(rolling_std.shape)

    # Create "circular time coordinate"
    # We select the last 28 days of the rolling std timeseries and we put them at the beginning of the rolling std timeseries
    # so we do not lose the first part smoothing with the rolling mean
    # the final time series must be 366 days long

    # select the last 28 days of the timeseries 
    last_days = rolling_std.isel(dayofyear=slice(-29, None)).compute() # Get the last 29 days dynamically
    #roll_std_last = rolling_std.sel(dayofyear=slice(338, 366)).compute()  # Get the last 29 days giving explicit position
    print(last_days.shape)
    # put the last days of the timeseries at the beginning, creating the "circular time coordinate"
    combined_array = xr.concat([last_days, rolling_std], dim='dayofyear').compute()
    print(combined_array.shape)
    # 30 days rolling mean of the standard deviation to smooth the signal fluctuations
    rolling_std_mean = combined_array.rolling(dayofyear=30).mean('dayofyear').compute()
    print(rolling_std_mean.shape)
    # drop the remaining nan in the timeseries
    rolling_std_mean = rolling_std_mean.dropna(dim='dayofyear').compute()
    print(rolling_std_mean.shape)
    # we average along lat lon coordinate (weighting with coslat) to have a unique signal applied to the whole domain
    spatial_mean_std = ctl.global_mean(rolling_std_mean)
    print(spatial_mean_std.shape)
    # we check if there are remaining nans just to be sure everything is right
    has_missing_values = np.isnan(spatial_mean_std).any().compute()
    print(has_missing_values)
    # we group the geopotential height anomalies by day of year and we divide them for the normalization coefficient
    anom_norm = (anom.groupby('time.dayofyear') / spatial_mean_std).compute()
    print(anom_norm.shape)
    # we transform the normalized anomalies in numpy to have them both in xarray and numpy if necessary
    anom_norm_np = anom_norm.values
    print(anom_norm_np.shape)
    # we check the presence of missing values
    has_missing_values = np.isnan(anom_norm_np).any()
    print(has_missing_values)
...
##############################################################
    ...
    # Detrend - optional
    # we eliminate a 50 years running means from the geopotential height anomalies after the normalization
    # Detrend - optional
    if detrend:
        print("Detrending anomalies with a running mean")
        # we compute the trend - use a window of the desired length and minimum period used for the computation
        z500_trend = anom_norm.rolling(min_periods=detrend_min_periods, time=detrend_time, center=True).mean().compute()
        # subtraction of the trend
        anom_norm = (anom_norm - z500_trend).compute()
        # conversion in numpy array to have both xarray and numpy
        anom_norm_np = anom_norm.values
        print(anom_norm_np.shape)
        # check the presence of missing values
        has_missing_values = np.isnan(anom_norm_np).any()
        print("Missing values after detrending:", has_missing_values)

################################################################

    # EOF calculation - optional
    ...
    if eof_calc:
        ...
        # Weight the data by sqrt of cos(lat)
        coslat = np.cos(np.deg2rad(anom_norm.coords['lat'].values)).clip(0., 1.)
        wgts = np.sqrt(coslat)[..., np.newaxis]
        # compute eof solver, eofs and pcs choosing the desired number of pcs
        # the solver uses a numpy array
        solver = Eof(anom_norm_np, weights=wgts)
        eof_np = solver.eofs(neofs=numpcs)
        pc_np = solver.pcs(npcs=numpcs, pcscaling=0)
        # compute the eigenvalues, variace for each pc and cumulative variance
        varfrac = solver.varianceFraction()
        lambdas = solver.eigenvalues()
        cumulative = varfrac.cumsum()

        print("EOFs shape:", eof_np.shape)
        print("PCs shape:", pc_np.shape)
        print("Explained variance:", varfrac[:numpcs])


#################################################################

        # Kmeans clustering - function used from scikit-learn

        if kmeans:
            print("Kmeans clustering")
            # parameters to initialize the kmeans clustering
            init = 'random'
            n_init = 600
            max_iter = 300
            random_state = None
            kms = KMeans(init=init, n_clusters = numclus, n_init=n_init, max_iter=max_iter, random_state=random_state).fit(pc_np)

            # centroids and labels given from the clustering
            kms_centroids_np = kms.cluster_centers_
            kms_labels_np = kms.labels_
            print(kms_centroids_np)
            print(kms_labels_np)


            # we insert the No Regime condition following Lee et. al. (2023)
            # we compute a centroid = 0 in the pc space and then we recompute the distance between the pcs
            # and new set of centroids to obtain the 7 wrs clusters + 1 No Regime condition cluster

            # we initialize an array of 0s having the same dimention as the other centroids
            noregime_centroid = np.zeros((1, kms_centroids_np.shape[1]))
            # we put all the centroids together
            wrs_centroids = np.vstack([kms_centroids_np, noregime_centroid])
            print(wrs_centroids.shape)
            # we recompute the distance between the pcs and the centroids
            distances = pairwise_distances(pc_np, wrs_centroids)
            print(distances.shape)
            # we obtain the new labels including the 7 regimes + NoRegime condition
            # taking the min distance between the pc and the centroid
            new_labels_np = np.argmin(distances, axis=1)
            print(new_labels_np)

            # transform a few stuff in xarray

            pc_np_dim = pc_np.shape[1]
            time_data = anom_norm['time']
            # pcs
            pc = xr.DataArray(pc_np, coords={'time': time_data, 'num': np.arange(pc_np_dim)}, dims=['time', 'num'])
            # wrs original kmeans labels
            kms_labels = xr.DataArray(kms_labels_np, coords={'time': time_data}, dims=['time'])
            # wrs labels with NoRegime condition
            new_labels = xr.DataArray(new_labels_np, coords={'time': time_data}, dims=['time'])
            print(pc.shape)
            print(kms_labels.shape)
            print(new_labels.shape)

            # we go back to the original measurement unit for geopot height [meters]
            # multiplying the pcs for the average normalization coefficient
            avg_norm_coeff = np.mean(spatial_mean_std)
            pc_denorm = pc * avg_norm_coeff
            print(pc_denorm)


            # we compute the clusters with the pcs in [meters]
            
            # Compute the clusters using the denormalized PCs
            clusters_denorm = {
                f"clust{i+1}_denorm": pc_denorm[new_labels_np == i].values
                for i in range(final_clus)
            }

            # Compute centroids for each cluster
            centroids_denorm = []
            for i in range(final_clus):
                cluster_name = f"clust{i+1}_denorm"
                
                # NoRegime (last cluster): set to zeros
                if i == numpcs:
                    centroid = np.zeros(numpcs)
                else:
                    cluster_data = clusters_denorm[cluster_name]
                    # Handle empty clusters safely (in case no data points fall into that cluster)
                    if cluster_data.size == 0:
                        print(f"Warning: Cluster {i+1} is empty. Filling with zeros.")
                        centroid = np.zeros(numpcs)
                    else:
            ...
            # Computing wrs patterns using function calculate_wr_period - see below
            print("Computing Weather Regimes patterns")
            wrs_denorm = calculate_wr_period(centroids_denorm, eof_np)
            print(np.min(wrs_denorm))
            print(np.max(wrs_denorm))

            return {
                'anom': anom_norm,
                'normalization_coefficient': spatial_mean_std,
                'eof': eof_np,
                'pc': pc,
                'varfrac': varfrac,
                'eigenvalues': lambdas,
                'cumulative_var': cumulative,
                'labels_original': kms_labels,
                'labels_extended': new_labels,
                'wr_year_denorm': wrs_denorm,
...
#############
    ...
    # proj_anom_np = solver.projectField(anom_norm_np, neofs = numpcs )
