...
cart_in = '/home/fabiano/data/SPHINX/tas_pr_mon/'
cart_in_3d = '/home/fabiano/data/SPHINX/va_ua_ta_mon/'
...
for varna in varss:
    ...
    for ens in ensmemok[:3]:
        plt.plot(allyears, globalme[(ens, varna)], label = ens, color = 'grey', linestyle = '-', linewidth = 0.7)
...
for varna in varss:
    for i, ann in enumerate(annme):
        fig = plt.figure()
        plt.title('Zonal mean {}: {}-{}'.format(ann-5, ann+5))
        cset = ctl.color_set(len(seasons)+1, bright_thres = 1)
        for seas, col in zip(seasons+['year'], cset):
            plt.plot(lat, zonal[('base', varna, seas)][i], label = 'base', linestyle = '-', color = col)
            plt.plot(lat, zonal[('stoc', varna, seas)][i], label = 'stoc', linestyle = '--', color = col)
        plt.legend()
        plt.grid()
        plt.xlabel('Lat')
        plt.ylabel(ylab)
...
for varna in varss:
    ...
    for seas, col in zip(seasons+['year'], cset):
        ...
        plt.plot(annme, mea, label = 'base', linestyle = '-', color = col)
        mea = [ctl.band_mean_from_zonal(zonal[('stoc', varna, seas)], lat, 70., 90.)]
        plt.plot(annme, mea, label = 'stoc', linestyle = '--', color = col)
    plt.legend()
    plt.grid()
    plt.xlabel('Year')
    plt.ylabel(ylab)
...
figure_file = cart_out + 'globalzonal_lcs0_vs_lcb0.pdf'
ctl.plot_pdfpages(figure_file, figures)

figure_file = cart_out + 'diffmaps_lcs0_vs_lcb0.pdf'
figure_maps = []
for varna in varss:
    for seas in ['year']+seasons:
        for i, ann in enumerate(annme):
            fig = ctl.plot_map_contour(climat[('stoc', varna, seas)][i]-climat[('base', varna, seas)][i], lat, lon, title = 'diff stoc-base {} {}: {}-{}'.format(varna, seas, ann-5, ann+5))
            figure_maps.append(fig)
ctl.plot_pdfpages(figure_file, figure_maps)

figure_file = cart_out + 'diffcross_lcs0_vs_lcb0.pdf'
figure_cross = []
for varna in varss3d:
    for seas in ['year']+seasons:
        for i, ann in enumerate(annme):
            fig = ctl.plot_lat_crosssection(cross3d[('stoc', varna, seas)][i]-cross3d[('base', varna, seas)][i], lat, lon, title = 'diff stoc-base {} {}: {}-{}'.format(varna, seas, ann-5, ann+5))
...
for coso in ['base', 'stoc']:
    ...
    for varna in varss:
        for seas in ['year']+seasons:
            for i, ann in enumerate(annme):
                fig = ctl.plot_map_contour(climat[(coso, varna, seas)][i], lat, lon, title = coso+' {} {}: {}-{}'.format(varna, seas, ann-5, ann+5))
                figure_maps.append(fig)

    figure_file = cart_out + '{}_maps_lcs0_vs_lcb0.pdf'.format(coso)
    ctl.plot_pdfpages(figure_file, figure_maps)
    ...
    for varna in varss3d:
        for seas in ['year']+seasons:
            for i, ann in enumerate(annme):
                fig = ctl.plot_lat_crosssection(cross3d[(coso, varna, seas)][i], lat, lon, title = coso+' {} {}: {}-{}'.format(varna, seas, ann-5, ann+5))
                figure_cross.append(fig)

    figure_file = cart_out + '{}_cross_lcs0_vs_lcb0.pdf'.format(coso)
    ctl.plot_pdfpages(figure_file, figure_cross)
